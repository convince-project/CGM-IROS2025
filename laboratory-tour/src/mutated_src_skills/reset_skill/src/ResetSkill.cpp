...
/* XXX: original code was :       request->field_name = convert<decltype(request->field_name)>(eventParams["field_name"].toString().toStdString()); */
      request->field_name = convert<decltype(request->field_name)>(eventParams["\0field_name"].toString().toStdString());
      bool wait_succeded{true};
      int retries = 0;
